...
public class PayPalPaymentSuccessNotificationHandler {
  ...
  public PaymentResponse handlePaymentResponse(IShoppingCart cart, ActionForm form, HttpServlet servlet,
      HttpServletRequest request, HttpServletResponse response) {
    // TODO Auto-generated method stub
    PayPalPaymentResponse pr = new PayPalPaymentResponse();
    try {
      

      String path = servlet.getServletContext().getRealPath(servlet.getInitParameter("paypalPaymentEmailTemplate"));
      ...
      mailUtil.setTo(billAddr.getEmail());  
      mailUtil.send();
      pr.setSuccess(true);
      logger.msg(getClass(), "handlePayment", "payment for order :[" +order+"] from Customer: ["+billAddr.getEmail()+"] processed successfuly]");
    } catch (IOException e) {
      pr.setException(e);
      pr.setSuccess(false);
...
//    item.setQuantity(1);
//    cart.addItem(item);
//    String path="C:/projects/ifaru/email/PayPalEmailTemplate.html";
//    PayPalPaymentSuccessNotificationHandler handler = new PayPalPaymentSuccessNotificationHandler();
//    Order order = new Order();
//    order.setOrderNumber(123456);
//    Customer customer = new Customer();
//    Address billingAddress = new Address();
//    billingAddress.setEmail("gr8buyer@buybuy.com");
