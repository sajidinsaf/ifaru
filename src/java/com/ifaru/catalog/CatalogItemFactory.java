...
public class CatalogItemFactory {
  ...
  private CatalogItems createFromXML(File data, boolean ignoreDisabled) throws IOException, SAXException, IFaruException {
    ...
    groupItems(catalogItems);
    return catalogItems;
  }
  
  // group items of the same type (id ending with a character)
  private void groupItems(CatalogItems catalogItems) {
    
    List<CatalogItem> items = new ArrayList<CatalogItem>();
    for (CatalogItem item: catalogItems.getItems()) {
      if (isSubItem(item.getId())) {
        addSizesToTopItem(item, items, catalogItems);
      } else if (!items.contains(item)) { // the top level item (item without the id ending with a character
        items.add(item);
      }
      
    }
    catalogItems.getItems().clear();
    catalogItems.getItems().addAll(items);
  }

  private void addSizesToTopItem(CatalogItem item, List<CatalogItem> newItems,
      CatalogItems origItems) {
    String topId = item.getId().substring(0, item.getId().length()-1);
    List<Size> sizes = item.getSizes();    
    CatalogItem origItem = null;
    for (CatalogItem newItem : newItems) {
      if (newItem.getId().equals(topId)) {
        newItem.getSizes().addAll(sizes);
        origItem = newItem;
        break;
      }
    }
    if (origItem == null) {
      for (CatalogItem newItem : origItems.getItems()) {
        if (newItem.getId().equals(topId)) {
          newItem.getSizes().addAll(sizes);
          origItem = newItem;
  ...
  // check if the id ends with a character
  private boolean isSubItem(String id) {
    if (id == null) {
      return false;
    }
    char lastChar = id.toCharArray()[id.length()-1];
